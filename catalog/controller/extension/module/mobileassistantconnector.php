...
abstract class BaseControllerModuleMobileAssistantConnector extends Controller
{
    ...
    public function __construct($registry)
    {
        ...
        $this->load->helper('emagicone/mobileassistant/constants');
        $this->load->helper('emagicone/mobileassistant/helper');
        $this->load->helper('emagicone/mobileassistant/setting');
        $this->load->helper('emagicone/mobileassistant/event');
    }
    ...
    public function index()
    {
        ...
        $this->s = EmagiconeMobileassistantSettingHelper::getSetting(
            $this->db,
            EmagiconeMobileassistantConstants::MODULE_SETTING_CODE
        );
...
//        }
        ...
        EmagiconeMobileassistantEventHelper::checkAndAddEvents($this, EmagiconeMobileassistantConstants::AREA_FRONTEND);
        EmagiconeMobileassistantHelper::createTables($this->db);
...
//                $this->generate_output('auth_error');
    ...
    {
        $html = '<h2>Mobile Assistant Connector (v. ' . EmagiconeMobileassistantConstants::MODULE_VERSION . ')</h2>';
        ...
        $html .= '<div style="margin-top: 15px; font-size: 13px;">Mobile Assistant Connector by <a href="http://emagicone.com" target="_blank" style="color: #15428B">eMagicOne</a></div>';
    ...
    private function get_qr_code()
    {
        ...
        $html .= '<h3>Mobile Assistant Connector (v. ' . EmagiconeMobileassistantConstants::MODULE_VERSION . ')</h3>';
    ...
    private function generate_output($data)
    {
        ...
        if (in_array(
            $this->call_function,
            array("test_config", "get_store_title", "get_store_stats", "get_data_graphs", "get_version"))
        ) {
        ...
        if ($add_bridge_version) {
            $data['module_version'] = EmagiconeMobileassistantConstants::MODULE_CODE;
    ...
    private function set_order_action()
    {
        ...
        if ($this->action == 'change_status') {
            ...
            if (EmagiconeMobileassistantHelper::isCartVersion20()) {
                ...
                if (version_compare(EmagiconeMobileassistantHelper::getCartVersion(), '1.5.4.1', '<=')) {
...
//        $s = $this->model_mobileassistant_setting->getSetting('mobassist');
        $s = EmagiconeMobileassistantSettingHelper::getSetting(
            $this->db,
            EmagiconeMobileassistantConstants::MODULE_SETTING_CODE
        );
...
//        $this->model_mobileassistant_setting->editSetting('mobassist', $s);
        EmagiconeMobileassistantSettingHelper::editSetting(
            $this->db,
            EmagiconeMobileassistantConstants::MODULE_SETTING_CODE,
            $s
        );
    ...
    public function push_new_order_156x($order_id, $total = 0)
    {
        ...
        if (!isset($order['order_status'])) {
            if ($order['order_status_id'] == 0) {
                ...
                $sql = "SELECT name FROM " . DB_PREFIX . "order_status WHERE language_id = '" . $this->getAdminLanguageId() . "' AND order_status_id = '" . $order['order_status_id'] . "'";
...
//        $this->check_version();
        if (version_compare(EmagiconeMobileassistantHelper::getCartVersion(), '2.2.0.0', '<')) {
            $order_id = $route;
        } elseif (version_compare(EmagiconeMobileassistantHelper::getCartVersion(), '2.3.0.0', '>=')) {
            $order_id = $response[0];
        } else {
            if (self::DEBUG_MODE) {
                $this->load->model('mobileassistant/helper');
...
//        return $s && isset($s['mobassist_status']) && $s['mobassist_status'] == 1;
        return !empty($s);
    }
...
//            $this->model_mobileassistant_setting->editSetting('mobassist', $s);
    ...
    private function _checkUpdateModule() {
        ...
        if (isset($s['mobassist_module_code']) && $s['mobassist_module_code'] < EmagiconeMobileassistantConstants::MODULE_CODE) {
            EmagiconeMobileassistantHelper::updateModule($this->db, $s);
            EmagiconeMobileassistantEventHelper::checkAndAddEvents(
                $this,
                EmagiconeMobileassistantConstants::AREA_FRONTEND
            );
...
//                'mobassist_status' => $s['mobassist_status'],
                'mobassist_module_code' => EmagiconeMobileassistantConstants::MODULE_CODE,
                'mobassist_module_version' => EmagiconeMobileassistantConstants::MODULE_VERSION,
